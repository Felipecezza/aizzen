    ...
    return (
      <Alert className="bg-dark-700 border-zinc-800">
        ...
        <AlertDescription className="text-zinc-400">
          Conecte sua primeira conta Logzz usando o formulário acima.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {webhooks.map((webhook) => {
        const fullWebhookUrl = `https://vtcchttcvmaijvjjcxpu.functions.supabase.co/universal-webhook${webhook.webhook_url}`;
        const webhookId = webhook.webhook_url.split('/').pop() || '';
        
        return (
          <Card key={webhook.id} className="bg-dark-700 border-zinc-800">
