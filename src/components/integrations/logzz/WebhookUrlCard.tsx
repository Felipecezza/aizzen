...
export const WebhookUrlCard = ({ onConnect, isLoading }: WebhookUrlCardProps) => {
  ...
  const fetchWebhooks = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq("integration_type", "logzz"); // Filtrar apenas webhooks do tipo 'logzz'
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          filter: 'integration_type=eq.logzz' // Filtrar apenas webhooks do tipo 'logzz'
        },
        (payload) => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-white">Logzz</h2>
        <p className="text-muted-foreground mt-2">
          Conecte sua conta Logzz para sincronizar seus pedidos automaticamente.
        </p>
      </div>
      ...
      <Card className="bg-dark-700 border-zinc-800">
        <CardHeader>
          ...
          <CardDescription className="text-zinc-400">
            Configure um novo webhook para receber pedidos da Logzz.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-2">
            <Input
              placeholder="Nome da integração (ex: Logzz Principal)"
      ...
      <Card className="bg-dark-700 border-zinc-800">
        <CardHeader>
          ...
          <CardDescription className="text-zinc-400">
            Gerencie suas integrações com a Logzz.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <WebhookList
